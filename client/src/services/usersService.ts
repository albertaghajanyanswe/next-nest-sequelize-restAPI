...
const usersService = {
  createUser: async <T>({
    ...
    return decodeCreateUser<T>(
      await usersApi.usersControllerRegisterGuest({ data })
    );
  },
  ...
  getCurrentUser: async <T>(params?: any) => {
    ...
    return decodeGetCurrentUser(
      await usersApi.usersControllerGetCurrentUser(params)
    );
  },
};
