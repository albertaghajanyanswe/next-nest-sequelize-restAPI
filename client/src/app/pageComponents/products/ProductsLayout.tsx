...
function ProductsLayout<T>({
        ...
        if (
          ...
          ((!Array.isArray(fieldValue) && fieldValue !== undefined) ||
            (Array.isArray(fieldValue) &&
              JSON.stringify(fieldValue) !== JSON.stringify(oldValue)))
        ) {
          newFilter = {
            ...newFilter,
            params: {
  ...
  const handleDeleteFavorite = async (
    ...
    try {
      e.stopPropagation();
      const res = details?.favoriteProducts?.find(
        (i: FavoriteProductDto) => i.storedProductId === details.id
      );
      if (res) {
        ...
        SystemMessage(enqueueSnackbar, getMessage(t, 'Something went wrong'), {
          variant: 'warning',
          theme,
        });
      }
    } catch (error: any) {
      SystemMessage(enqueueSnackbar, getMessage(error), {
        variant: 'error',
        theme,
      });
    }
  ...
  const calculateIsFavorite = (product: ProductsDataType) => {
    ...
    const res = product?.favoriteProducts?.find(
      (i: FavoriteProductDto) => i.storedProductId === product.id
    );
    return !!res;
  };
    ...
    <Box sx={muiStyles.root}>
      ...
      <Box sx={muiStyles.tableRoot}>
        {tableSources.data.length === 0 && !loading ? (
          <EmptyState
            title={t('emptyProductTitle')}
            desc={t('emptyProductDesc')}
          />
        ) : (
          <>
