...
export function testStepsCoverage(
  ...
  return {
    missing: Object.keys(defaults).filter(
      (name) => !names[name] && !(name in extra)
    ),
...
const StepHOC =
  <T,>() =>
  <L extends keyof T & string>(STEPS: readonly L[]) => {
    const Form = genForm<Pick<T, L>>();
    const GeneratorFunc = <Props,>(
      Component: React.FC<
        Props & { Form: ReturnType<typeof genForm<Pick<T, L>>> }
      >
    ) => {
      const WrappedComponent: MemoExoticComponent<
