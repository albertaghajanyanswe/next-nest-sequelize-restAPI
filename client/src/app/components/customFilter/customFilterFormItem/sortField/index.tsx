...
import AscSVG from '@/assets/select/filter-asc.svg';
import DescSvg from '@/assets/select/filter-desc.svg';
...
function CustomSortField<T extends iFilterSortField>({
  ...
  const [sortState, setSortState] = React.useState<{
    ...
    order: (sortValue?.find((i) => i === 'asc' || i === 'desc') || '') as
      | 'asc'
      | 'desc'
      | '',
  });
  ...
  const handleOnChang = (event: SelectChangeEvent<string[]>) => {
    ...
    if (last === 'asc') {
      ...
      const idx = valueCopy.findIndex(
        (item) => item !== 'asc' && item !== 'desc' && item !== last
      );
      if (idx !== -1) {
    ...
    const orderBy = (valueCopy.find((i) => i === 'asc' || i === 'desc') ||
      '') as 'asc' | 'desc' | '';
  ...
  const rerenderOptions = () => {
    ...
    const r = optionsList?.map((opt: any) =>
      opt.type === 'group' ? (
        <ListSubheader key={opt.name} sx={muiStyles.listSubheader}>
          {opt.label}
        </ListSubheader>
      ) : opt.type === 'separator' ? (
        <Box sx={{ p: '4px 12px 0 12px' }} key={opt.name}>
        ...
        <Select
          ...
          value={
            [
              ...
              ...(sortState.order ? [sortState.order] : []),
            ] || []
          }
              ...
              ? (selected: string[] = []) => {
                  ...
                  if (Array.isArray(selected)) {
                    ...
                    return (
                      ...
                      >
                        {result.includes('Ascending') ? (
                          <Box sx={{ display: 'flex', mr: 1 }}>
                            <AscSVG />
                          </Box>
                        ) : result.includes('Descending') ? (
                          <Box sx={{ display: 'flex', mr: 1 }}>
                            <DescSvg />
                          </Box>
                        ) : null}
                        {result.find(
                          (i) => i !== 'Ascending' && i !== 'Descending'
                        ) || ''}
                      </Box>
                    );
                  }
                  return (
                    ...
                    >
                      {(selected as string[]).includes('Ascending') ? (
                        <Box sx={{ display: 'flex', mr: 1 }}>
                          <AscSVG />
                        </Box>
                      ) : (selected as string[]).includes('Descending') ? (
                        <Box sx={{ display: 'flex', mr: 1 }}>
                          <DescSvg />
                        </Box>
                      ) : null}
                      {(selected as string[]).find(
                        (i) => i !== 'Ascending' && i !== 'Descending'
                      )}
                    </Box>
                  );
        ...
        >
          {!isMultiple && (
            <MenuItem value="" key="empty" sx={{ height: '40px' }}>
              ...
              >
                <div>{field.insteadNone || t('none')}</div>
              </Box>
            </MenuItem>
          )}
          {rerenderOptions()}
