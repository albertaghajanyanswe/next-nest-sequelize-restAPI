...
const useQueryParams = ({
  ...
  const pageParams = qs.parse(location && location.search.split('?')[1], {
  ...
  const tableFilter = useRef({
    params: {
      sort: (pageParams?.sort ||
        (pageDefaultParams !== undefined
          ? pageDefaultParams.params.sort
          : { field: 'id', order: 'asc' })) as unknown as iSort,
      filter: (pageParams?.filter ||
        (pageDefaultParams !== undefined
          ? pageDefaultParams.params.filter
          : {})) as unknown as iFilter,
      limit:
        Number(pageParams?.limit) ||
        (pageDefaultParams !== undefined ? pageDefaultParams.params.limit : 10),
      skip:
        Number(pageParams?.skip) ||
        (pageDefaultParams !== undefined ? pageDefaultParams.params.skip : 0),
      search: (Number(pageParams?.search) ||
        (pageDefaultParams !== undefined
          ? pageDefaultParams.params.search
          : {})) as unknown as iSearch,
    },
  } as iFilterParams);
